...
public class LocationMapController : SubWindowController
{
    ...
    public Sprite CurrentMap { get; set; }

    public override void Show()
    {
        if (CurrentMap == null)
            throw new Exception("Current map null");
    ...
    public void OpenMap(Sprite sprite)
    {
        ...
        if (spriteWidth > spriteHeight && newImageSize.y < 216)
        {
            ...
            _viewPortTransform.anchoredPosition =
                new Vector2(_viewPortTransform.anchoredPosition.x, newImageSize.y / 2);
        }
        else
        {
